...
export default function createPressData(
  ...
  for (const item of pages) {
    ...
    if (item.isDirectory()) {
      ...
      const nestedFolder = createPressData(path.join(source, item.name), generateIndex, generateSearchIndex, true);
      // If folder is draft, continue
      // This includes all sub-folders and their files inside the folder
      if (nestedFolder[0].get("default").size > 0 && nestedFolder[0].get("default").get("draft")) continue;
