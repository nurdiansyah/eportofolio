...
if (fs.existsSync("./src/lib/SveltePress/theme/meta/package.json")) {
  ...
  const themePakcageJson = JSON.parse(fs.readFileSync("./src/lib/SveltePress/theme/meta/package.json", "utf8"));

  ["devDependencies", "dependencies"].forEach((x) => {
    if (themePakcageJson.hasOwnProperty(x)) packageJson[x] = { ...packageJson[x], ...themePakcageJson[x] };
  });
...
if (fs.existsSync("./src/lib/SveltePress/theme/index.svelte"))
  fs.writeFileSync("./src/routes/index.svelte", fs.readFileSync("./src/lib/SveltePress/theme/index.svelte", "utf8"));
  ...
  if (typeof fs.cpSync === "function") {
    ...
    function copyFiles(source) {
      ...
      for (const item of files) {
        ...
        const rootRelative = join("./", relative("./src/lib/SveltePress/theme/meta/root/", fullPath));
        if (item.isDirectory()) {
